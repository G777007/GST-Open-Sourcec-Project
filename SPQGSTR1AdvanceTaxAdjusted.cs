...
{
    public partial class SPQGSTR1AdvanceTaxAdjusted : Form
    {
        r1Publicclass objGSTR1A5 = new r1Publicclass();

        public SPQGSTR1AdvanceTaxAdjusted()
        {
            InitializeComponent();
            // SET GRID PROPERTY
            SetGridViewColor();
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    var result = (from r in dt.AsEnumerable()
                                  ...
                                  select new
                                  {
                                      ...
                                      colPOS = g.Key.colPOS,
                                      colAdvadj = g.Sum(x => Convert.ToString(x["Fld_Advadj"]).Trim() == "" ? 0 : Convert.ToDecimal(x["Fld_Advadj"])),
                    ...
                    {
                        dgvGSTR1A5.Rows.Add("", i, item.colRate, item.colPOS, Utility.DisplayIndianCurrency(Convert.ToString(item.colAdvadj)), Utility.DisplayIndianCurrency(Convert.ToString(item.colIGST)), Utility.DisplayIndianCurrency(Convert.ToString(item.colCGST)), Utility.DisplayIndianCurrency(Convert.ToString(item.colSGST)), Utility.DisplayIndianCurrency(Convert.ToString(item.colCess)));
                        i++;
                    }
                    Application.DoEvents();
        ...
        public void SetGridViewColor()
        {
            try
            {
                ...
                this.Width = Convert.ToInt32(Screen.PrimaryScreen.WorkingArea.Width * (0.57));
                this.Height = Convert.ToInt32(Screen.PrimaryScreen.WorkingArea.Height * (0.77));

                // set width of header, main and total grid
                this.panel1.Width = Convert.ToInt32(Screen.PrimaryScreen.WorkingArea.Width * (0.554));
                this.dgvGSTR1A5.Width = Convert.ToInt32(Screen.PrimaryScreen.WorkingArea.Width * (0.554));
                this.dgvGSTR1A5Total.Width = Convert.ToInt32(Screen.PrimaryScreen.WorkingArea.Width * (0.554));
                ...
                this.pbClose.Location = new System.Drawing.Point(Convert.ToInt32(Screen.PrimaryScreen.WorkingArea.Width * (0.53)), 2);
