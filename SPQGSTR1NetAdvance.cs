...
{
    public partial class SPQGSTR1NetAdvance : Form
    {
        r1Publicclass objGSTR1A5 = new r1Publicclass();

        public SPQGSTR1NetAdvance()
        {
            InitializeComponent();
            // SET GRID PROPERTY
            SetGridViewColor();
            ...
            GetData();
            // TOTAL CALCULATION
            string[] colNo = { "colInvoiceNo", "colARNNo", "colInvoiceValue", "colIGSTAmnt", "colCGSTAmnt", "colSGSTAmnt", "colCessAmount" };
            GetTotal(colNo);
            ...
            dgvGSTR1A5.Columns[6].Visible = false;
            dgvGSTR1A5.Columns[7].Visible = false;
            ...
            dgvGSTR1A5Total.Columns[6].Visible = false;
            dgvGSTR1A5Total.Columns[7].Visible = false;
        }
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        for (int j = 0; j < dt.Columns.Count; j++)
                        {
                            ...
                            if (ColName == "colInvoiceValue" || ColName == "colIGSTAmnt" || ColName == "colCGSTAmnt" || ColName == "colSGSTAmnt" || ColName == "colCessAmount")
            ...
            try
            {
                if (dgvGSTR1A5.Rows.Count > 1)
                {
                    ...
                    if (dgvGSTR1A5Total.Rows.Count == 0)
                    {
                        ...
                        dr["colTARNNo"] = dgvGSTR1A5.Rows.Cast<DataGridViewRow>().Where(x => Convert.ToString(x.Cells["colARNNo"].Value).Trim() != "").GroupBy(x => x.Cells["colARNNo"].Value).Select(x => x.First()).Distinct().Count();
                    ...
                    else if (dgvGSTR1A5Total.Rows.Count == 1)
                    {
                        ...
                        foreach (var item in colNo)
                        {
                            ...
                            if (item == "colARNNo")
                                dgvGSTR1A5Total.Rows[0].Cells["colTARNNo"].Value = dgvGSTR1A5.Rows.Cast<DataGridViewRow>().Where(x => Convert.ToString(x.Cells["colARNNo"].Value).Trim() != "").GroupBy(x => x.Cells["colARNNo"].Value).Select(x => x.First()).Distinct().Count();
                            ...
                            else
                            {
                                ...
                                if (line.Length > 0)
                                {
                                    ...
                                    for (int i = 0; i < sCells.GetLength(0); ++i)
                                    {
                                        ...
                                        if (iCol + i < this.dgvGSTR1A5.ColumnCount && i < 13)
                                        {
        ...
        private void GridRowPaste(DataTable dtDGV, int lineNo, int iCol, string[] lines)
        {
            try
            {
                ...
                foreach (string line in lines)
                {
                    ...
                    if (cnt >= lineNo)
                    {
                        if (line != "" && line.Length > 0)
                        {
                            ...
                            {
                                // CHECK GRID COLUMN COUNT
                                if (iCol + i < this.dgvGSTR1A5.ColumnCount && colNo < 14)
                                {
                                    // SKIP CHECK BOX COLUMN AND SEQUANCE COLUMN TO PASTE DATA
                                    if (iCol == 0)
                                        colNo = iCol + i + 2;
                ...
                // TOTAL CALCULATION
                string[] colGroup = { "colInvoiceNo", "colARNNo", "colInvoiceValue", "colIGSTAmnt", "colCGSTAmnt", "colSGSTAmnt", "colCessAmount" }; ;
                GetTotal(colGroup);
        ...
        public bool IsValidateData()
        {
            ...
            try
            {
                ...
                list = (
                    from x in this.dgvGSTR1A5.Rows.OfType<DataGridViewRow>()
                    where !Utility.IsBlankInvoiceNumber(Convert.ToString(x.Cells["colInvoiceNo"].Value))
                    select x).ToList<DataGridViewRow>();
                if (list.Count > 0)
                {
                    for (i = 0; i < list.Count; i++)
                    {
                        this.dgvGSTR1A5.Rows[list[i].Cells["colInvoiceNo"].RowIndex].Cells["colInvoiceNo"].Style.BackColor = Color.Red;
                    }
                    _cnt++;
                    _str = string.Concat(_str, _cnt, ") Please enter proper Invoice Number.\n");
                }
                list = (
                    from x in this.dgvGSTR1A5.Rows.OfType<DataGridViewRow>()
                    where Utility.IsBlankInvoiceNumber(Convert.ToString(x.Cells["colInvoiceNo"].Value))
                ...
                for (i = 0; i < list.Count; i++)
                {
                    this.dgvGSTR1A5.Rows[list[i].Cells["colInvoiceNo"].RowIndex].Cells["colInvoiceNo"].Style.BackColor = Color.White;
                }
                list = null;
                list = (
                    from x in this.dgvGSTR1A5.Rows.OfType<DataGridViewRow>()
                ...
                list = (
                    from x in this.dgvGSTR1A5.Rows.OfType<DataGridViewRow>()
                    where !Utility.IsBlankDate(Convert.ToString(x.Cells["colARNDate"].Value))
                    select x).ToList<DataGridViewRow>();
                if (list.Count > 0)
                {
                    for (i = 0; i < list.Count; i++)
                    {
                        this.dgvGSTR1A5.Rows[list[i].Cells["colARNDate"].RowIndex].Cells["colARNDate"].Style.BackColor = Color.Red;
                    }
                    _cnt++;
                    _str = string.Concat(_str, _cnt, ") Please enter proper ARN /Document Date.\n");
                }
                list = (
                    from x in this.dgvGSTR1A5.Rows.OfType<DataGridViewRow>()
                    where Utility.IsBlankDate(Convert.ToString(x.Cells["colARNDate"].Value))
                ...
                for (i = 0; i < list.Count; i++)
                {
                    this.dgvGSTR1A5.Rows[list[i].Cells["colARNDate"].RowIndex].Cells["colARNDate"].Style.BackColor = Color.White;
                }
                list = null;
                list = (
                    from x in this.dgvGSTR1A5.Rows.OfType<DataGridViewRow>()
                ...
                if (list.Count > 0)
                {
                    ...
                    _cnt++;
                    _str = string.Concat(_str, _cnt, ") Please enter proper Advance adjusted value.\n");
                }
                list = (
                    from x in this.dgvGSTR1A5.Rows.OfType<DataGridViewRow>()
                    where Utility.IsInvoiceValue(Convert.ToString(x.Cells["colInvoiceValue"].Value))
                ...
                if (!(_str != ""))
                {
                    if (this.objGSTR1A5.InsertValidationFlg("GSTR1", "NET-ADVANCE", "true", CommonHelper.SelectedMonth) != 1)
                    {
                ...
                else
                {
                    ...
                    if (this.objGSTR1A5.InsertValidationFlg("GSTR1", "NET-ADVANCE", "false", CommonHelper.SelectedMonth) != 1)
                    {
                        MessageBox.Show("System error.\nPlease try after sometime! SPQValidation Error", "System Error", MessageBoxButtons.OK, MessageBoxIcon.Hand);
                    }
        ...
        public bool IsValidateData_old()
        {
            try
            {
                ...
                dgvGSTR1A5.AllowUserToAddRows = false;

                pbGSTR1.Visible = true;

                #region GSTIN Number
                List<DataGridViewRow> list = dgvGSTR1A5.Rows
                       .OfType<DataGridViewRow>()
                       .Where(x => true != Utility.IsBlankGSTN(Convert.ToString(x.Cells["colGSTIN"].Value)))
                       .Select(x => x)
                       .ToList();
                if (list.Count > 0)
                {
                    ...
                    {
                        dgvGSTR1A5.Rows[list[i].Cells["colGSTIN"].RowIndex].Cells["colGSTIN"].Style.BackColor = Color.Red;
                    }
                    _cnt += 1;
                    _str += _cnt + ") Please enter proper GSTIN Number.\n";
                }
                list = dgvGSTR1A5.Rows
                       .OfType<DataGridViewRow>()
                       .Where(x => true == Utility.IsBlankGSTN(Convert.ToString(x.Cells["colGSTIN"].Value)))
                       .Select(x => x)
                       .ToList();
                for (int i = 0; i < list.Count; i++)
                {
                    dgvGSTR1A5.Rows[list[i].Cells["colGSTIN"].RowIndex].Cells["colGSTIN"].Style.BackColor = Color.White;
                }
                #endregion
                ...
                list = dgvGSTR1A5.Rows
                       .OfType<DataGridViewRow>()
                       .Where(x => true != Utility.IsBlankInvoiceNumber(Convert.ToString(x.Cells["colInvoiceNo"].Value)))
                       .Select(x => x)
                       .ToList();
                if (list.Count > 0)
                {
                    for (int i = 0; i < list.Count; i++)
                    {
                        dgvGSTR1A5.Rows[list[i].Cells["colInvoiceNo"].RowIndex].Cells["colInvoiceNo"].Style.BackColor = Color.Red;
                    }
                    _cnt += 1;
                    _str += _cnt + ") Please enter proper Invoice Number.\n";
                }
                list = dgvGSTR1A5.Rows
                       .OfType<DataGridViewRow>()
                       .Where(x => true == Utility.IsBlankInvoiceNumber(Convert.ToString(x.Cells["colInvoiceNo"].Value)))
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    dgvGSTR1A5.Rows[list[i].Cells["colInvoiceNo"].RowIndex].Cells["colInvoiceNo"].Style.BackColor = Color.White;
                }
                #endregion
                ...
                list = dgvGSTR1A5.Rows
                       .OfType<DataGridViewRow>()
                       .Where(x => true != Utility.IsBlankDate(Convert.ToString(x.Cells["colInvoiceDate"].Value)))
                       .Select(x => x)
                       .ToList();
                if (list.Count > 0)
                {
                    for (int i = 0; i < list.Count; i++)
                    {
                        dgvGSTR1A5.Rows[list[i].Cells["colInvoiceDate"].RowIndex].Cells["colInvoiceDate"].Style.BackColor = Color.Red;
                    }
                    _cnt += 1;
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    dgvGSTR1A5.Rows[list[i].Cells["colInvoiceDate"].RowIndex].Cells["colInvoiceDate"].Style.BackColor = Color.White;
                }
                #endregion
                ...
                list = dgvGSTR1A5.Rows
                       .OfType<DataGridViewRow>()
                       .Where(x => true != Utility.IsBlankDate(Convert.ToString(x.Cells["colARNDate"].Value)))
                       .Select(x => x)
                       .ToList();
                if (list.Count > 0)
                {
                    for (int i = 0; i < list.Count; i++)
                    {
                        dgvGSTR1A5.Rows[list[i].Cells["colARNDate"].RowIndex].Cells["colARNDate"].Style.BackColor = Color.Red;
                    }
                    _cnt += 1;
                    _str += _cnt + ") Please enter proper ARN /Document Date.\n";
                }
                list = dgvGSTR1A5.Rows
                       .OfType<DataGridViewRow>()
                       .Where(x => true == Utility.IsBlankDate(Convert.ToString(x.Cells["colARNDate"].Value)))
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    dgvGSTR1A5.Rows[list[i].Cells["colARNDate"].RowIndex].Cells["colARNDate"].Style.BackColor = Color.White;
                }
                #endregion
                ...
                if (list.Count > 0)
                {
                    for (int i = 0; i < list.Count; i++)
                    {
                        dgvGSTR1A5.Rows[list[i].Cells["colInvoiceValue"].RowIndex].Cells["colInvoiceValue"].Style.BackColor = Color.Red;
                    }
                    _cnt += 1;
                    _str += _cnt + ") Please enter proper Advance adjusted value.\n";
                }
                list = dgvGSTR1A5.Rows
                       .OfType<DataGridViewRow>()
                       .Where(x => true == Utility.IsInvoiceValue(Convert.ToString(x.Cells["colInvoiceValue"].Value)))
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    dgvGSTR1A5.Rows[list[i].Cells["colInvoiceValue"].RowIndex].Cells["colInvoiceValue"].Style.BackColor = Color.White;
                }
                #endregion
                ...
                list = dgvGSTR1A5.Rows
                       .OfType<DataGridViewRow>()
                       .Where(x => true != Utility.IsRate(Convert.ToString(x.Cells["colIGSTRate"].Value)))
                       .Select(x => x)
                       .ToList();
                if (list.Count > 0)
                {
                    for (int i = 0; i < list.Count; i++)
                    {
                        dgvGSTR1A5.Rows[list[i].Cells["colIGSTRate"].RowIndex].Cells["colIGSTRate"].Style.BackColor = Color.Red;
                    }
                    _cnt += 1;
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    dgvGSTR1A5.Rows[list[i].Cells["colIGSTRate"].RowIndex].Cells["colIGSTRate"].Style.BackColor = Color.White;
                }
                #endregion
                ...
                {
                    string pgst = Convert.ToString(dgvGSTR1A5.Rows[list[j].Cells["colPOS"].RowIndex].Cells["colPOS"].Value);
                    string result1 = pgst;

                    if (result.ToLower() != result1.ToLower())
                    {
                        if (Convert.ToString(dgvGSTR1A5.Rows[list[j].Cells["colIGSTAmnt"].RowIndex].Cells["colIGSTAmnt"].Value) == "")
                        {
                            _cnt += 1;
                            _str += _cnt + ") Please enter proper IGST Amount.\n";
                            dgvGSTR1A5.Rows[list[j].Cells["colIGSTAmnt"].RowIndex].Cells["colIGSTAmnt"].Style.BackColor = Color.Red;
                        }
                        else
                        {
                            if (!Utility.IsICSC(Convert.ToString(dgvGSTR1A5.Rows[list[j].Cells["colIGSTAmnt"].RowIndex].Cells["colIGSTAmnt"].Value)))
                            {
                                _cnt += 1;
                                _str += _cnt + ") Please enter proper IGST Amount.\n";
                                dgvGSTR1A5.Rows[list[j].Cells["colIGSTAmnt"].RowIndex].Cells["colIGSTAmnt"].Style.BackColor = Color.Red;
                                ...
                                dgvGSTR1A5.Rows[list[j].Cells["colIGSTAmnt"].RowIndex].Cells["colIGSTAmnt"].Style.BackColor = Color.White;
                        }
                    }
                    else if (Convert.ToString(dgvGSTR1A5.Rows[list[j].Cells["colIGSTAmnt"].RowIndex].Cells["colIGSTAmnt"].Value) != "")
                    {
                        _cnt += 1;
                        _str += _cnt + ") Please enter proper IGST Amount.\n";
                        dgvGSTR1A5.Rows[list[j].Cells["colIGSTAmnt"].RowIndex].Cells["colIGSTAmnt"].Style.BackColor = Color.Red;
                ...
                for (int j = 0; j < list.Count; j++)
                {
                    ...
                    if (result.ToLower() == result1.ToLower())
                    {
                        if (Convert.ToString(dgvGSTR1A5.Rows[list[j].Cells["colCGSTAmnt"].RowIndex].Cells["colCGSTAmnt"].Value) == "")
                        {
                            _cnt += 1;
                            _str += _cnt + ") Please enter proper CGST Amount.\n";
                            dgvGSTR1A5.Rows[list[j].Cells["colCGSTAmnt"].RowIndex].Cells["colCGSTAmnt"].Style.BackColor = Color.Red;
                        }
                        else
                        {
                            if (!Utility.IsICSC(Convert.ToString(dgvGSTR1A5.Rows[list[j].Cells["colCGSTAmnt"].RowIndex].Cells["colCGSTAmnt"].Value)))
                            {
                                _cnt += 1;
                                _str += _cnt + ") Please enter proper CGST Amount.\n";
                                dgvGSTR1A5.Rows[list[j].Cells["colCGSTAmnt"].RowIndex].Cells["colCGSTAmnt"].Style.BackColor = Color.Red;
                                ...
                                dgvGSTR1A5.Rows[list[j].Cells["colCGSTAmnt"].RowIndex].Cells["colCGSTAmnt"].Style.BackColor = Color.White;
                        }
                    }
                    else if (Convert.ToString(dgvGSTR1A5.Rows[list[j].Cells["colCGSTAmnt"].RowIndex].Cells["colCGSTAmnt"].Value) != "")
                    {
                        _cnt += 1;
                        _str += _cnt + ") Please enter proper CGST Amount.\n";
                        dgvGSTR1A5.Rows[list[j].Cells["colCGSTAmnt"].RowIndex].Cells["colCGSTAmnt"].Style.BackColor = Color.Red;
                ...
                for (int j = 0; j < list.Count; j++)
                {
                    ...
                    if (result.ToLower() == result1.ToLower())
                    {
                        if (Convert.ToString(dgvGSTR1A5.Rows[list[j].Cells["colSGSTAmnt"].RowIndex].Cells["colSGSTAmnt"].Value) == "")
                        {
                            _cnt += 1;
                            _str += _cnt + ") Please enter proper SGST Amount.\n";
                            dgvGSTR1A5.Rows[list[j].Cells["colSGSTAmnt"].RowIndex].Cells["colSGSTAmnt"].Style.BackColor = Color.Red;
                        }
                        else
                        {
                            if (!Utility.IsICSC(Convert.ToString(dgvGSTR1A5.Rows[list[j].Cells["colSGSTAmnt"].RowIndex].Cells["colSGSTAmnt"].Value)))
                            {
                                _cnt += 1;
                                _str += _cnt + ") Please enter proper SGST Amount.\n";
                                dgvGSTR1A5.Rows[list[j].Cells["colSGSTAmnt"].RowIndex].Cells["colSGSTAmnt"].Style.BackColor = Color.Red;
                                ...
                                dgvGSTR1A5.Rows[list[j].Cells["colSGSTAmnt"].RowIndex].Cells["colSGSTAmnt"].Style.BackColor = Color.White;
                        }
                    }
                    else if (Convert.ToString(dgvGSTR1A5.Rows[list[j].Cells["colSGSTAmnt"].RowIndex].Cells["colSGSTAmnt"].Value) != "")
                    {
                        _cnt += 1;
                        _str += _cnt + ") Please enter proper SGST Amount.\n";
                        dgvGSTR1A5.Rows[list[j].Cells["colSGSTAmnt"].RowIndex].Cells["colSGSTAmnt"].Style.BackColor = Color.Red;
                ...
                for (int j = 0; j < list.Count; j++)
                {
                    if (Convert.ToString(dgvGSTR1A5.Rows[list[j].Cells["colCessAmount"].RowIndex].Cells["colCessAmount"].Value) != "")
                    {
                        if (!Utility.IsICSC(Convert.ToString(dgvGSTR1A5.Rows[list[j].Cells["colCessAmount"].RowIndex].Cells["colCessAmount"].Value)))
                        {
                            _cnt += 1;
                            _str += _cnt + ") Please enter proper CESS Amount.\n";
                            dgvGSTR1A5.Rows[list[j].Cells["colCessAmount"].RowIndex].Cells["colCessAmount"].Style.BackColor = Color.Red;
                        }
                        else
                        { dgvGSTR1A5.Rows[list[j].Cells["colCessAmount"].RowIndex].Cells["colCessAmount"].Style.BackColor = Color.White; }
                    }
                    else
                    { dgvGSTR1A5.Rows[list[j].Cells["colCessAmount"].RowIndex].Cells["colCessAmount"].Style.BackColor = Color.White; }
                }
                ...
                if (list.Count > 0)
                {
                    for (int i = 0; i < list.Count; i++)
                    {
                        dgvGSTR1A5.Rows[list[i].Cells["colCGSTAmnt"].RowIndex].Cells["colCGSTAmnt"].Style.BackColor = Color.Red;
                        dgvGSTR1A5.Rows[list[i].Cells["colSGSTAmnt"].RowIndex].Cells["colSGSTAmnt"].Style.BackColor = Color.Red;
                    }
                    _cnt += 1;
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    if (dgvGSTR1A5.Rows[list[i].Cells["colCGSTAmnt"].RowIndex].Cells["colCGSTAmnt"].Style.BackColor != Color.Red)
                        dgvGSTR1A5.Rows[list[i].Cells["colCGSTAmnt"].RowIndex].Cells["colCGSTAmnt"].Style.BackColor = Color.White;
                    if (dgvGSTR1A5.Rows[list[i].Cells["colSGSTAmnt"].RowIndex].Cells["colSGSTAmnt"].Style.BackColor != Color.Red)
                        dgvGSTR1A5.Rows[list[i].Cells["colSGSTAmnt"].RowIndex].Cells["colSGSTAmnt"].Style.BackColor = Color.White;
                }
                #endregion
                ...
                list = dgvGSTR1A5.Rows
                       .OfType<DataGridViewRow>()
                       .Where(x => Convert.ToString(x.Cells["colCGSTAmnt"].Value) != "" && Convert.ToString(x.Cells["colSGSTAmnt"].Value) != "" && Convert.ToString(x.Cells["colIGSTRate"].Value) != "" && Convert.ToString(x.Cells["colInvoiceValue"].Value) != "" && x.Cells["colCGSTAmnt"].Style.BackColor == Color.White && x.Cells["colSGSTAmnt"].Style.BackColor == Color.White)
                       .Select(x => x)
                       .ToList();
                if (list.Count > 0)
                {
                    for (int i = 0; i < list.Count; i++)
                    {
                        decimal CGST = Convert.ToDecimal(dgvGSTR1A5.Rows[list[i].Cells["colCGSTAmnt"].RowIndex].Cells["colCGSTAmnt"].Value);
                        decimal SGST = Convert.ToDecimal(dgvGSTR1A5.Rows[list[i].Cells["colSGSTAmnt"].RowIndex].Cells["colSGSTAmnt"].Value);
                        decimal Rate = Convert.ToDecimal(dgvGSTR1A5.Rows[list[i].Cells["colIGSTRate"].RowIndex].Cells["colIGSTRate"].Value);
                        decimal Tax = Convert.ToDecimal(dgvGSTR1A5.Rows[list[i].Cells["colInvoiceValue"].RowIndex].Cells["colInvoiceValue"].Value);
                        ...
                        if (Convert.ToDecimal(dgvGSTR1A5.Rows[list[i].Cells["colCGSTAmnt"].RowIndex].Cells["colCGSTAmnt"].Value) == ComValue && Convert.ToDecimal(dgvGSTR1A5.Rows[list[i].Cells["colSGSTAmnt"].RowIndex].Cells["colSGSTAmnt"].Value) == ComValue)
                        {
                            if (dgvGSTR1A5.Rows[list[i].Cells["colCGSTAmnt"].RowIndex].Cells["colCGSTAmnt"].Style.BackColor != Color.Red)
                                dgvGSTR1A5.Rows[list[i].Cells["colCGSTAmnt"].RowIndex].Cells["colCGSTAmnt"].Style.BackColor = Color.White;
                            if (dgvGSTR1A5.Rows[list[i].Cells["colSGSTAmnt"].RowIndex].Cells["colSGSTAmnt"].Style.BackColor != Color.Red)
                ...
                list = dgvGSTR1A5.Rows
                       .OfType<DataGridViewRow>()
                       .Where(x => Convert.ToString(x.Cells["colIGSTAmnt"].Value) != "" && Convert.ToString(x.Cells["colIGSTRate"].Value) != "" && Convert.ToString(x.Cells["colInvoiceValue"].Value) != "" && x.Cells["colIGSTAmnt"].Style.BackColor == Color.White)
                       .Select(x => x)
                       .ToList();
                if (list.Count > 0)
                {
                    for (int i = 0; i < list.Count; i++)
                    {
                        decimal IGST = Convert.ToDecimal(dgvGSTR1A5.Rows[list[i].Cells["colIGSTAmnt"].RowIndex].Cells["colIGSTAmnt"].Value);
                        decimal Rate = Convert.ToDecimal(dgvGSTR1A5.Rows[list[i].Cells["colIGSTRate"].RowIndex].Cells["colIGSTRate"].Value);
                        decimal Tax = Convert.ToDecimal(dgvGSTR1A5.Rows[list[i].Cells["colInvoiceValue"].RowIndex].Cells["colInvoiceValue"].Value);
                        ...
                        if (Convert.ToDecimal(dgvGSTR1A5.Rows[list[i].Cells["colIGSTAmnt"].RowIndex].Cells["colIGSTAmnt"].Value) == ComValue)
                        {
                            if (dgvGSTR1A5.Rows[list[i].Cells["colIGSTAmnt"].RowIndex].Cells["colIGSTAmnt"].Style.BackColor != Color.Red)
                                dgvGSTR1A5.Rows[list[i].Cells["colIGSTAmnt"].RowIndex].Cells["colIGSTAmnt"].Style.BackColor = Color.White;
                        }
                        else
                        {
                            dgvGSTR1A5.Rows[list[i].Cells["colIGSTAmnt"].RowIndex].Cells["colIGSTAmnt"].Style.BackColor = Color.Red;
                            _cnt += 1;
                ...
                */
                ...
                if (list.Count > 0)
                {
                    for (int i = 0; i < list.Count; i++)
                    {
                        dgvGSTR1A5.Rows[list[i].Cells["colPOS"].RowIndex].Cells["colPOS"].Style.BackColor = Color.Red;
                    }
                    _cnt += 1;
                ...
                list = dgvGSTR1A5.Rows
                       .OfType<DataGridViewRow>()
                       .Where(x => true == Utility.IsNumber(Convert.ToString(x.Cells["colPOS"].Value)))
                       .Select(x => x)
                       .ToList();
                for (int i = 0; i < list.Count; i++)
                {
                    dgvGSTR1A5.Rows[list[i].Cells["colPOS"].RowIndex].Cells["colPOS"].Style.BackColor = Color.White;
                }
                #endregion

                dgvGSTR1A5.AllowUserToAddRows = true;
                pbGSTR1.Visible = false;

                if (_str != "")
                {
                    CommonHelper.StatusText = "Draft";
                    int _Result = objGSTR1A5.InsertValidationFlg("GSTR1", "NET-ADVANCE", "false", CommonHelper.SelectedMonth);
                    if (_Result != 1)
                ...
                {
                    int _Result = objGSTR1A5.InsertValidationFlg("GSTR1", "NET-ADVANCE", "true", CommonHelper.SelectedMonth);
                    if (_Result != 1)
                        MessageBox.Show("System error.\nPlease try after sometime! SPQValidation Error", "System Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private Boolean chkCellValue(string cellValue, int cNo)
        {
            try
            {
                if (cellValue.Trim() != "" || cNo == 17)// NOT EQUEL BLANK
                {
                    ...
                    else if (cNo == 8 || cNo == 10 || cNo == 11 || cNo == 12 || cNo == 13) // value
                    {
        ...
        private void dgvGSTR1A5_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (e.RowIndex >= 0)
                {
                    if (cNo == 2 || cNo == 5 || cNo == 7 || cNo == 9 || dgvGSTR1A5.Columns[cNo].Name == "colPOS")
                    {
                    ...
                    else if (cNo == 4 || cNo == 6 || cNo == 8 || cNo == 10 || cNo == 11 || cNo == 12 || cNo == 13) // value
                    {
                        if (chkCellValue(Convert.ToString(dgvGSTR1A5.Rows[e.RowIndex].Cells[cNo].Value).Trim(), cNo))
                        {
                            if (cNo != 4 && cNo != 6)
        ...
        public void Save()
        {
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    _Result = objGSTR1A5.GSTR1_NetAdvance(dt, Convert.ToString(CommonHelper.StatusText));
                    ...
                    if (_Result == 1)
                    {
                        ...
                        _Result = objGSTR1A5.GSTR1_NetAdvance(dt, "Total");
                        if (_Result == 1)
                        {
                            //DONE
            ...
            using (OleDbConnection con = new OleDbConnection(conn))
            {
                try
                {
                    try
                    {
                        ...
                        OleDbDataAdapter oleAdpt = new OleDbDataAdapter("select * from [advance_tax_adjusted$]", con);
                    ...
                    if (dtexcel != null && dtexcel.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dtexcel.Rows.Count; i++)
                        {
                            ...
                            sj = dtexcel.Columns["colARNDate"].Ordinal;
                            dtexcel = Utility.ChangeColumnDataType(dtexcel, dtexcel.Columns["colARNDate"].ColumnName, typeof(string));
                            dtexcel.Columns["colARNDate"].SetOrdinal(sj);

                            try
                            {
                                DateTime ss = Convert.ToDateTime(dtexcel.Rows[i]["colARNDate"]);
                                dtexcel.Rows[i]["colARNDate"] = Convert.ToString(ss.ToString("dd-MM-yyyy").Replace('/', '-'));
                            }
                            catch (Exception)
                            {
                                dtexcel.Rows[i]["colARNDate"] = "";
                            }
                        }
                        dtexcel.AcceptChanges();
                        #endregion
        ...
        public void ExportExcel()
        {
            try
            {
                if (dgvGSTR1A5.Rows.Count > 1)
                {
                    ...
                    newWS.Name = "advance_tax_adjusted";

                    // DELETE UNUSED WORKSHEETS FROM WORKBOOK
                    foreach (Excel.Worksheet ws in WB.Worksheets)
                    {
                        if (ws.Name != "advance_tax_adjusted")
                    ...
                    if (CommonHelper.IsLicence)
                    {
                        ...
                        for (int i = 0; i < dgvGSTR1A5.Rows.Count - 1; i++)
                        {
                            ...
                            {
                                if (dgvGSTR1A5.Columns[j].Name == "colInvoiceDate" || dgvGSTR1A5.Columns[j].Name == "colARNDate")
                                {
                                    try
                                    {
        ...
        public void ExportExcelForValidatation()
        {
            ...
            try
            {
                if (dgvGSTR1A5.Rows.Count > 1)
                {
                    ...
                    ((Excel.Range)newWS.Cells[1, 15]).ColumnWidth = 45;
                    ...
                    for (int k = 0; k < dgvGSTR1A5.Rows.Count; k++)
                    {
                        ...
                        for (int j = 0; j < dgvGSTR1A5.ColumnCount; j++)
                        {
                            if (dgvGSTR1A5.Rows[k].Cells[j].Style.BackColor == Color.Red)
                            {
                                ...
                                if (dgvGSTR1A5.Columns[j].Name == "colIGSTAmnt")
                                {
                                    if (dgvGSTR1A5.Rows[k].Cells[j].Value.ToString() == "")
                                        str_error += cnt + ") " + " Please enter proper " + dgvGSTR1A5.Columns[j].HeaderText + ".\n";
                                    else
                                        str_error += cnt + ") " + dgvGSTR1A5.Columns[j].HeaderText + " is not applicable for Intra State. Please enter exact match " + dgvGSTR1A5.Columns[j].HeaderText + " base on `Advance adjusted` and `Rate` calculation. \n";
                                }
                                else if (dgvGSTR1A5.Columns[j].Name == "colCGSTAmnt")
                                {
                                    if (dgvGSTR1A5.Rows[k].Cells[j].Value.ToString() == "")
                                        ...
                                        str_error += cnt + ") " + dgvGSTR1A5.Columns[j].HeaderText + " is not applicable for Inter State. Please enter exact match " + dgvGSTR1A5.Columns[j].HeaderText + " base on `Advance adjusted` and `Rate` calculation or CGST & SGST values must be same. \n";
                                }
        ...
        public void JSONCreator()
        {
            try
            {
                ...
                if (CommonHelper.CurrentTurnOver != null && Convert.ToString(CommonHelper.CurrentTurnOver).Trim() != "")
                {
                    ...
                    dt.Columns.Remove("colARNNo");
                    dt.Columns.Remove("colARNDate");
                    dt.AcceptChanges();
                    #endregion
                    ...
                    if (lstPOS != null && lstPOS.Count > 0)
                    {
                        ...
                        List<Txpd> objLstAT = new List<Txpd>();
        ...
        public void SaveJson()
        {
            try
            {
                ...
                //List<string> list = dgvGSTR1A5.Rows
                //       .OfType<DataGridViewRow>()
                ...
                //    B2b b2bObj = new B2b();
                //    b2bObj.ctin = Convert.ToString(dgvGSTR1A5.Rows[0].Cells[2].Value);
                //    obj.Add(b2bObj);
                ...
                //        {
                //            #region Invoice Number
                //            List<DataGridViewRow> Invoicelist = dgvGSTR1A5.Rows
                //                   .OfType<DataGridViewRow>()
                ...
                //                    clsB2b.ctin = Convert.ToString(Invoicelist[j].Cells["colGSTIN"].Value);
                ...
                //                    {
                //                        clsItmDet.txval = Convert.ToInt32(Invoicelist[j].Cells["colInvoiceTaxableVal"].Value);
                //                    }
                //                    else { clsItmDet.txval = 0; }
                ...
                //                    {
                //                        clsItmDet.crt = Convert.ToInt32(Invoicelist[j].Cells["colCGSTRate"].Value);
                //                    }
                //                    else { clsItmDet.crt = 0; }
                //                    if (Convert.ToString(Invoicelist[j].Cells["colCGSTAmnt"].Value).Trim() != "")//CGST Amount
                //                    {
                ...
                //                    {
                //                        clsItmDet.srt = Convert.ToInt32(Invoicelist[j].Cells["colSGSTRate"].Value);
                //                    }
                //                    else { clsItmDet.srt = 0; }
                //                    if (Convert.ToString(Invoicelist[j].Cells["colSGSTAmnt"].Value).Trim() != "")//SGST Amount
                //                    {
                //                        clsItmDet.samt = Convert.ToDouble(Invoicelist[j].Cells["colSGSTAmnt"].Value);
                //                    }
                //                    else { clsItmDet.samt = 0; }
        ...
        public void SetGridViewColor()
        {
            try
            {
                ...
                this.dgvGSTR1A5.Width = Convert.ToInt32(Screen.PrimaryScreen.WorkingArea.Width * (0.96));
                this.dgvGSTR1A5Total.Width = Convert.ToInt32(Screen.PrimaryScreen.WorkingArea.Width * (0.96));
                ...
                //this.lnkback.Location = new System.Drawing.Point(Convert.ToInt32(Screen.PrimaryScreen.WorkingArea.Width * (0.93)), 5);
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ((DataTable)dgvGSTR1A5.DataSource).DefaultView.RowFilter = string.Format("colSequence like '%{0}%' or colGSTIN like '%{0}%' or colName like '%{0}%' or colInvoiceNo like '%{0}%' or colInvoiceDate like '%{0}%' or colARNNo like '%{0}%' or colARNDate like '%{0}%' or colInvoiceValue like '%{0}%' or colIGSTRate like '%{0}%' or colIGSTAmnt like '%{0}%' or colCGSTAmnt like '%{0}%' or colSGSTAmnt like '%{0}%' or colCessAmount like '%{0}%' or colPOS like '%{0}%'", txtSearch.Text.Trim().Replace("'", "''"));
                }
                else
                {
