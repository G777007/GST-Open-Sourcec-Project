...
{
    public partial class SPQGSTR2ACND : Form
    {
        ...
        private void GetData()
        {
            try
            {
                ...
                DataTable dt = new DataTable();
                string Query = "Select * from SPQR2ACND where Fld_Month='" + CommonHelper.SelectedMonth + "' AND Fld_FinancialYear = '" + CommonHelper.ReturnYear + "' and Fld_FileStatus != 'Total'";
                ...
                if (dgvGSTR27Other.Rows.Count > 0)
                {
                    ...
                    if (dgvGSTR27OtherTotal.Rows.Count == 0)
                    {
                        ...
                        foreach (DataGridViewColumn col in dgvGSTR27Other.Columns)
                        {
                            dt.Columns.Add(col.Name.ToString());
                            col.DataPropertyName = col.Name;
                            if (col.Name == "colOrginvoiceValue")
                                dt.Columns["colOrginvoiceValue"].DataType = typeof(System.Double);
                        }
                        #endregion
                        #region ASSIGN GRIDVIEW ROWS IN DATATABLE
                        ...
                        #region New
                        decimal NoteValue = 0, tax = 0, igst = 0, cgst = 0, sgst = 0, cess = 0;
                        double sum = 0;

                        #region CDN Value
                        var result2 = (from row in dt.AsEnumerable()
                                       where row.Field<double>("colOrginvoiceValue") != null && row.Field<string>("colGSTIN") != "" && row.Field<string>("colDbtCrdtNoteNo") != "" && row.Field<string>("colTypeOfNote") == "Debit Note"
                                       group row by new { colGSTIN = row.Field<string>("colGSTIN"), colNoteValue = row.Field<double>("colOrginvoiceValue"), colDbtCrdtNoteNo = row.Field<string>("colDbtCrdtNoteNo") } into grp
                                       select new
                                       {
                                           colGSTIN = grp.Key.colGSTIN,
                                           colOrginvoiceValue = grp.Key.colNoteValue,
                        ...
                        sum = InvoiceSum.AsEnumerable().Sum(row => Convert.ToDouble(row.Field<double>("colOrginvoiceValue")));

                        result2 = null;
                        result2 = (from row in dt.AsEnumerable()
                                   where row.Field<double>("colOrginvoiceValue") != null && row.Field<string>("colGSTIN") != "" && row.Field<string>("colDbtCrdtNoteNo") != "" && row.Field<string>("colTypeOfNote") == "Credit Note"
                                   group row by new { colGSTIN = row.Field<string>("colGSTIN"), colNoteValue = row.Field<double>("colOrginvoiceValue"), colDbtCrdtNoteNo = row.Field<string>("colDbtCrdtNoteNo") } into grp
                                   select new
                                   {
                                       colGSTIN = grp.Key.colGSTIN,
                        ...
                        sum = sum - InvoiceSum.AsEnumerable().Sum(row => Convert.ToDouble(row.Field<double>("colOrginvoiceValue")));

                        result2 = null;
                        result2 = (from row in dt.AsEnumerable()
                                   where row.Field<double>("colOrginvoiceValue") != null && row.Field<string>("colGSTIN") != "" && row.Field<string>("colDbtCrdtNoteNo") != "" && row.Field<string>("colTypeOfNote") == "Refund Voucher"
                                   group row by new { colGSTIN = row.Field<string>("colGSTIN"), colNoteValue = row.Field<double>("colOrginvoiceValue"), colDbtCrdtNoteNo = row.Field<string>("colDbtCrdtNoteNo") } into grp
                                   select new
                                   {
                                       colGSTIN = grp.Key.colGSTIN,
                        ...
                        if (sum != null)
                            dr["colTOrginvoiceValue"] = sum;
                        else
                            dr["colTOrginvoiceValue"] = 0;
                        #endregion
                       ...
                       .ToList();
                        ...
                        {
                            //NoteValue = list.Cast<DataGridViewRow>().Where(x => x.Cells["colNoteValue"].Value != null).Sum(x => x.Cells["colNoteValue"].Value.ToString().Trim() == "" ? 0 : Convert.ToDecimal(x.Cells["colNoteValue"].Value));
                       ...
                       .ToList();
                        if (list != null && list.Count > 0)
                        {
                            //NoteValue = NoteValue - list.Cast<DataGridViewRow>().Where(x => x.Cells["colNoteValue"].Value != null).Sum(x => x.Cells["colNoteValue"].Value.ToString().Trim() == "" ? 0 : Convert.ToDecimal(x.Cells["colNoteValue"].Value));
                    ...
                    {
                        #region IF TOTAL GRID HAVING ONLY ONE RECORDS Old
                        ...
                        #region IF TOTAL GRID HAVING ONLY ONE RECORDS New
                       ...
                       .ToList();
                        ...
                        foreach (var item in colNo)
                        {
                            if (item == "colDbtCrdtNoteNo" || item == "colOrginvoiceValue" || item == "colGSTIN")
                            {
                                DataTable dt = new DataTable();
                                #region ADD DATATABLE COLUMN
                                foreach (DataGridViewColumn col in dgvGSTR27Other.Columns)
                                ...
                                foreach (DataGridViewRow drn in dgvGSTR27Other.Rows)
                                {
                                    if (drn.Index != dgvGSTR27Other.Rows.Count)
                                    {
                                        rowValue[0] = "False";
                                        for (int i = 1; i < drn.Cells.Count; i++)
                                        {
                                ...
                                if (sum != null)
                                    dgvGSTR27OtherTotal.Rows[0].Cells["colTOrginvoiceValue"].Value = sum;
                                else
                                    dgvGSTR27OtherTotal.Rows[0].Cells["colTOrginvoiceValue"].Value = 0;
                                #endregion

                            }
                            else if (item == "colOrgInvoiceNo")
                                dgvGSTR27OtherTotal.Rows[0].Cells["colTOrgInvoiceNo"].Value = dgvGSTR27Other.Rows.Cast<DataGridViewRow>().Where(x => Convert.ToString(x.Cells["colOrgInvoiceNo"].Value).Trim() != "").GroupBy(x => x.Cells["colOrgInvoiceNo"].Value).Select(x => x.First()).Distinct().Count();
                            //else if (item == "colNoteValue")
                            //    dgvGSTR27OtherTotal.Rows[0].Cells["Column1"].Value = NoteValue;
                            else if (item == "colTaxable")
                ...
                if (e.KeyCode == Keys.Delete)
                {
                    ...
                    try
                    {
                        if (dgvGSTR27Other.Rows.Count > 0)
                        {
                            ...
                            {
                                if (oneCell.Selected && oneCell.ColumnIndex != 0 && !oneCell.ReadOnly)
                                {
        ...
        public void ExportExcel()
        {
            try
            {
                if (dgvGSTR27Other.Rows.Count > 0)
                {
                    ...
                    Excel.Range rg = rg = (Excel.Range)sheetRange.Cells[1, 6];
                    rg.EntireColumn.NumberFormat = "dd-MM-yyyy";

                    rg = rg = (Excel.Range)sheetRange.Cells[1, 5];
                    rg.EntireColumn.NumberFormat = "@";

                    rg = rg = (Excel.Range)sheetRange.Cells[1, 11];
                    rg.EntireColumn.NumberFormat = "dd-MM-yyyy";
